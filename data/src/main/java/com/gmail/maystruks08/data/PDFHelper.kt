...
class PDFHelper @Inject constructor(private val calendarHelper: CalendarHelper) {
    ...
    fun exportMenu(menu: Menu) {
        ...
        try {
            ...
            table.addCell(
                createPdfPCell(
                    "${menu.name} ${calendarHelper.format(
                        menu.dateOfStartMenu,
                        CalendarHelper.DATE_FORMAT
                    )}.${menu.startFrom.title} старт раскладки", Element.ALIGN_CENTER, true
                )
            )
    ...
    fun exportPurchaseListGroupByProduct(fileName: String, data: List<ShoppingListItem>) {
        val document = createDocument("СписокПокупокПоПродуктам_$fileName")
        try {
    ...
    fun exportPurchaseListGroupByStoreDepartment(fileName: String, data: Map<StoreDepartment, List<ShoppingListItem>>) {
        val document = createDocument("СписокПокупокПоОтделам_$fileName")
        try {
            val table = PdfPTable(2)
            table.setWidths(intArrayOf(2, 1))
            table.addCell(createPdfPCell("Продукты", Element.ALIGN_LEFT, true))
            table.addCell(createPdfPCell("Вес", Element.ALIGN_RIGHT, true))
            data.forEach { entry ->
                val cellHeader = createPdfPCell(entry.key.type, Element.ALIGN_CENTER, true).apply { colspan = 2 }
    ...
    private fun createDayTable(day: Day): PdfPTable {
        return PdfPTable(3).apply {
            val cellHeader = createPdfPCell(
                "День ${day.number} ${calendarHelper.format(
                    day.date,
                    CalendarHelper.DATE_FORMAT
                )}. Вес на одного ${day.getDayTotalWeightForOne()}. Вес на всех ${day.getDayTotalWeightForAll()}",
    ...
    private fun createFoodReceiptTable(typeOfMeal: TypeOfMeal, products: List<Product>): PdfPTable {
        return PdfPTable(3).apply {
            ...
            products.forEach { product ->
                ...
                addCell(
                    createPdfPCell(
                        product.portion.portionForAllPeople.toString(),
                        Element.ALIGN_RIGHT
                    )
                )
    ...
    private fun createDocument(fileName: String): Document {
        return Document(PageSize.A3, 50f, 50f, 50f, 50f).apply {
            try {
